		...
		if ( typeof selector === "string" ) {
			...
			if ( match && (match[1] || !context) ) {
				...
				if ( match[1] ) {
					...
					document = (context ? context.ownerDocument || context : document);
					...
					if ( ret ) {
						...
						ret = jQuery.buildFragment( [ match[1] ], [ document ] );
...
jQuery.event = {
	...
	fix: function( event ) {
		...
		if ( event.pageX == null && event.clientX != null ) {
			...
			event.pageX = event.clientX + (document && document.scrollLeft || body && body.scrollLeft || 0) - (document && document.clientLeft || body && body.clientLeft || 0);
			event.pageY = event.clientY + (document && document.scrollTop  || body && body.scrollTop  || 0) - (document && document.clientTop  || body && body.clientTop  || 0);
		}
...
jQuery.buildFragment = function( args, nodes, scripts ) {
	var fragment, cacheable, cacheresults,
		document = (nodes && nodes[0] ? nodes[0].ownerDocument || nodes[0] : document);

	// Only cache "small" (1/2 KB) HTML strings that are associated with the main document
	// Cloning options loses the selected state, so don't cache them
	// IE 6 doesn't like it when you put <object> or <embed> elements in a fragment
	// Also, WebKit does not clone 'checked' attributes on cloneNode, so don't cache
	if ( args.length === 1 && typeof args[0] === "string" && args[0].length < 512 && document === document &&
	...
	if ( !fragment ) {
		...
		jQuery.clean( args, document, fragment, scripts );
	}
